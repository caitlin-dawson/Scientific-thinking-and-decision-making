...
Note about string processing: some versions/packages in R have difficulty with the character encoding of the Finnish characters ä and ö. Where text strings are processed, two versions of the code are given--check which version you need for the character encoding you're using. 
...
# Save CI task data
#write.csv(data_citiT,file='data_citiT.csv', row.names=FALSE)
...
NB! Strings have changed due to updating to R version  4.2.3 (2023-03-15 ucrt) which reads the characters ä and ö correctly. The original strings are commented out but kept in case of users with other versions of R. 
...
data_nonvalid <- data_Q_total %>%
  filter(Validity == "Älä huomioi aineistoani. Jokin muu syy esti minua osallistumasta kunnolla." | Validity == "Älä huomioi aineistoani. En suurimmaksi osaksi keskittynyt tai lukenut kysymyksiä kunnolla.")
...
data_Q_lang_omit <- data_Q_total %>%
  filter(Language != "Sujuva / äidinkieli" & Language != "Keskitaso / keskusteleva" & Language != "Muu, mikä?")
...
data_Q_total <- data_Q_total %>%
  filter(Validity %in% c("Aineistoani voi käyttää.", "Muu, mikä? ", NA, "") & Language %in% c("Sujuva / äidinkieli", "Keskitaso / keskusteleva", "Muu, mikä?"))
...
# McDonald's omegas; NOT 100% SURE ABOUT THESE
library(GPArotation)
summary(omega(data_Q_total[, epicur[1:5]]))
summary(omega(data_Q_total[, epicur[6:10]]))
...
# McDonald's omegas
summary(omega(data_Q_total[, IH[1:5]]))
summary(omega(data_Q_total[, IH[6:10]]))
summary(omega(data_Q_total[, IH[11:16]]))
summary(omega(data_Q_total[, IH[17:22]]))
...
# McDonald's omegas
summary(omega(data_Q_total[, need_clo]))
...
# McDonald's omegas
summary(omega(data_Q_total[, need_cog]))
...
# McDonald's omega
summary(omega(data_Q_total[, aot]))
...
sci_att1 <- data_Q_total %>%
     select(sa.1a, sa.1b, sa.1d, sa.1g)
sci_att2 <- data_Q_total %>%
     select(sa.1c, sa.1e, sa.1h, sa.1f)
sci_att3 <- data_Q_total %>%
     select(sa.2a, sa.2b, sa.2c, sa.2d)


psych::alpha(sci_att1)
psych::alpha(sci_att2)
psych::alpha(sci_att3)

#as character vectors
sci_att1 <- c("sa.1a", "sa.1b", "sa.1d", "sa.1g")
sci_att2 <- c("sa.1c", "sa.1e", "sa.1h", "sa.1f")
sci_att3 <- c("sa.2a", "sa.2b", "sa.2c", "sa.2d")

# McDonald's omegas
summary(omega(data_Q_total[, sci_att1]))
summary(omega(data_Q_total[, sci_att2]))
summary(omega(data_Q_total[, sci_att3]))
...
# McDonald's omega
summary(omega(data_Q_total[, sci_cur]))
...
NB: Notice that there are two versions. Choose the correct one based on string processing. 
...
# Q1
data_Q_total$hr.1 <- as.factor(data_Q_total$hr.1)
levels(data_Q_total$hr.1) <- list("N" = "Kumpikin on yhtä todennäköistä", "H-R" = "Kuudes lapsi on tyttö", "Neither" = "Kuudes lapsi on poika")
...
# Q2
data_Q_total$hr.2 <- as.factor(data_Q_total$hr.2)
levels(data_Q_total$hr.2) <- list("N" = "Kummatkin sarjat ovat yhtä todennäköisiä", "H-R" = "THHTHT", "Neither" = "HTHTHT")
...
# 3.1
data_Q_total$hr.3.1 <- as.factor(data_Q_total$hr.3.1)
levels(data_Q_total$hr.3.1) <- list("b" = "Herra F. sai sydänkohtauksen", "a" = "Herra F. on yli 55-vuotias ja on saanut sydänkohtauksen", "c" = "Herra F.:llä on suuri perhe")
...
# 3.2
data_Q_total$hr.3.2 <- as.factor(data_Q_total$hr.3.2)
levels(data_Q_total$hr.3.2) <- list("b" = "Herra F. sai sydänkohtauksen", "a" = "Herra F. on yli 55-vuotias ja on saanut sydänkohtauksen", "c" = "Herra F.:llä on suuri perhe")
...
# 3.3
data_Q_total$hr.3.3 <- as.factor(data_Q_total$hr.3.3)
levels(data_Q_total$hr.3.3) <- list("b" = "Herra F. sai sydänkohtauksen", "a" = "Herra F. on yli 55-vuotias ja on saanut sydänkohtauksen", "c" = "Herra F.:llä on suuri perhe")
...
# Q4
data_Q_total$hr.4 <- as.factor(data_Q_total$hr.4)
levels(data_Q_total$hr.4) <- list("H-E" = "Kumpikin on yhtä tehokas", "N" = "Kognitiivis-behavioraalista terapiaa", "Neither" = "Lääkehoitoa")
# Kognitiivis-behavioraalista terapiaa = N; Yhta tehokas = H-E
...
# Q6
data_Q_total$hr.6 <- as.factor(data_Q_total$hr.6)
levels(data_Q_total$hr.6) <- list("N" = "Tyttö", "H-E" = "Kumpikin on yhtä todennäköistä", "Neither" = "Poika")
# Tytto = N; Kumpikin on yht? todenn?k?ist? = H-E
...
# Create binary heuristic scores (0=no heuristic responses (most normative, but includes incorrect), 1=at least one heuristic response) 
...
#HE: 61.04% make at least one heuristic response 
#HR: 59.09% make at least one heuristic response 
...
data_Q_total <- data_Q_total %>%
  mutate(rp.1.int = case_when(rp.1 == "Sairaalassa B (jossa syntyy 10 lasta päivässä)." ~ 1,
                              rp.1 %in% c("Sairaalassa A (jossa syntyy 50 lasta päivässä).", "Tämä on yhtä todennäköistä kummassakin sairaalassa.") ~ 0)) 
...
Update and characters affects this section
...
data_matreasT <- data_matreasT %>%
  select(Participant.Private.ID, Spreadsheet:ANSWER) %>% 
  filter(display %in% c("Tehtävä_6", "Tehtävä_8"))
...
data_Q_lang_omit_matrix <- data_matreasT_final %>%
  filter(Language != "Sujuva / äidinkieli" & Language != "Keskitaso / keskusteleva" & Language != "Muu, mikä?")
data_matreasT_final <- data_matreasT_final %>%
  filter(Validity.quantised %in% c("1", NA, "") & Language %in% c("Sujuva / äidinkieli", "Keskitaso / keskusteleva", "Muu, mikä?"))
...
* If two variables are highly correlated, one of them is dropped (Eisenberg et al., 2018) (r > .85)
...
* The reaction time for each row is replaced with missing values (NAs) if it is equal to or higher than 1025ms (screen time 1000ms + 16.67ms refresh rate + ~8ms latency between computer and mouse -> reaction times higher than this are likely not valid) *or* if it is equal to or lower than 150ms.
...
# write.csv(data_ConsentValidity, "C:/Users/Omistaja/OneDrive/Tiedostot/GitHub/Scientific-thinking/data_ConsentValidity.csv", row.names = FALSE)
#write.csv(data_ConsentValidity,file='data_ConsentValidity.csv', row.names=FALSE)
...
write.csv(data_efa,file='data_efa.csv', row.names=FALSE)
...
#subset minus IDs, binary variables, variables we didn't use, and the factor scores if they've been combined
corset<-data_efa %>%
  select(-Participant.Private.ID,-HEscore,-HRscore,-MeanGoRT, -SDGoRTlog)
#  select(-Participant.Private.ID,-HEscore,-HRscore,-MeanGoRT, -SDGoRTlog,-f1,-f2,-f3,-f4)

##make a latex table with correlations
library(xtable)
#http://myowelt.blogspot.com/2008/04/beautiful-correlation-tables-in-r.html
corstarsl <- function(corset){
require(Hmisc)
corset <- as.matrix(corset)
R <- rcorr(corset)$r
p <- rcorr(corset)$P
...
## trunctuate the matrix that holds the correlations to two decimals
R <- format(round(cbind(rep(-1.11, ncol(corset)), R), 2))[,-1]

## build a new matrix that includes the correlations with their appropriate stars
Rnew <- matrix(paste(R, mystars, sep=""), ncol=ncol(corset))
diag(Rnew) <- paste(diag(R), " ", sep="")
rownames(Rnew) <- colnames(corset)
colnames(Rnew) <- paste(colnames(corset), "", sep="")
...
# Combine mean/SD matrix and correlation matrix

means <- colMeans(corset, na.rm = TRUE)
sds <- apply(corset, 2, sd, na.rm = TRUE)
means <- round(means, 2)
sds <- round(sds, 2)
# Combine means and SD into a matrix
mean_sd_matrix <- data.frame(means, sds)
#calculate correlation matrix with stars for significant correlations
cors<-corstarsl(corset[,1:26])
#combine the dfs
output_matrix <- cbind(mean_sd_matrix, cors)

##Save the output as a CSV file
write.csv(output_matrix, "current_cors.csv")

xtable(corstarsl(corset[,1:26])) #Latex code
