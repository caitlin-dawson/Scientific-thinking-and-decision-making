...
b5_rev <- c(which(colnames(data_Q_total) == "agreeablenessReverse"),
            which(colnames(data_Q_total) == "emotionalStabilityReverse"),
            which(colnames(data_Q_total) == "extroversionReverse"),
            which(colnames(data_Q_total) == "conscientiousnessReverse"),
            which(colnames(data_Q_total) == "opennessReverse"))
...
data_Q_total <- data_Q_total %>% 
  mutate(B5_omit = case_when(B5_max >= .95 | END.QUESTIONNAIRE.BIG5 <= 1000*length(b5) ~ 1,
                             B5_max < .95 & END.QUESTIONNAIRE.BIG5 > 1000*length(b5) ~ 0))
 
# Omit B5 data based on the B5_omit column
data_Q_total[which(data_Q_total$B5_omit == 1), b5] <- NA
# NOTE: if we want to omit participants with only NAs, we use B5_omit != 0
...
# Shapiro-Wilks test
shapiro_p_b5 <- data.frame(name = rep(0, length(b5)), 
                           p_value = rep(0, length(b5)))
for (i in 1:length(b5)) {
...
# Shapiro-Wilks test
shapiro_p_b5_mean <- data.frame(name = rep(0, length(b5_mean)), 
                                p_value = rep(0, length(b5_mean)))
for (i in 1:length(b5_mean)) {
...
# Shapiro-Wilks test
shapiro_p_epicur <- data.frame(name = rep(0, length(epicur)), 
                               p_value = rep(0, length(epicur)))
for (i in 1:length(epicur)) {
...
# Factor/sum score level assumptions
...
describe(data_Q_total[, c(epicur_sum)])[, c("mean", "skew", "kurtosis")]

# Shapiro-Wilks tests
shapiro_p_epicur_sum <- data.frame(name = rep(0, length(epicur_sum)), 
                                   p_value = rep(0, length(epicur_sum)))
for (i in 1:length(epicur_sum)) {
  shapiro_p_epicur_sum$name[i] <- colnames(data_Q_total[epicur_sum[i]])
  shapiro_p_epicur_sum$p_value[i] <- shapiro.test(data_Q_total[, epicur_sum[i]])["p.value"][[1]]
}
...
IH <- c(which(colnames(data_Q_total) == "IH1rev1"),
        which(colnames(data_Q_total) == "IH2rev1"),
        which(colnames(data_Q_total) == "IH3rev1"),
        which(colnames(data_Q_total) == "IH4rev1"),
        which(colnames(data_Q_total) == "IH5rev1"),
        which(colnames(data_Q_total) == "IH6norm2"),
        which(colnames(data_Q_total) == "IH7norm2"),
        which(colnames(data_Q_total) == "IH8norm2"),
        which(colnames(data_Q_total) == "IH9norm2"),
        which(colnames(data_Q_total) == "IH10norm2"),
        which(colnames(data_Q_total) == "IH11norm3"),
        which(colnames(data_Q_total) == "IH12norm3"),
        which(colnames(data_Q_total) == "IH13norm3"),
        which(colnames(data_Q_total) == "IH14norm3"),
        which(colnames(data_Q_total) == "IH15norm3"),
        which(colnames(data_Q_total) == "IH16norm3"),
        which(colnames(data_Q_total) == "IH17rev4"),
        which(colnames(data_Q_total) == "IH18rev4"),
        which(colnames(data_Q_total) == "IH19rev4"),
        which(colnames(data_Q_total) == "IH20rev4"),
        which(colnames(data_Q_total) == "IH21rev4"),
        which(colnames(data_Q_total) == "IH22rev4"))
IH_rev <- c(which(colnames(data_Q_total) == "IH1rev1"),
            which(colnames(data_Q_total) == "IH2rev1"),
            which(colnames(data_Q_total) == "IH3rev1"),
            which(colnames(data_Q_total) == "IH4rev1"),
            which(colnames(data_Q_total) == "IH5rev1"),
...
data_Q_total <- data_Q_total %>% 
  mutate(IH_omit = case_when(IH_max >= .95 | END.QUESTIONNAIRE.INT.HUM <= 1000*length(IH) ~ 1,
                             IH_max < .95 & END.QUESTIONNAIRE.INT.HUM > 1000*length(IH) ~ 0))
 
# Omit IH data based on the IH_omit column
data_Q_total[which(data_Q_total$IH_omit == 1), IH] <- NA
# NOTE: if we want to omit participants with only NAs, we use IH_omit != 0
...
# Describe: skew, kurtosis, mean
describe(data_Q_total[, IH])[, c("mean", "skew", "kurtosis")]
# Shapiro-Wilks test
shapiro_p_IH <- data.frame(name = rep(0, length(IH)), p_value = rep(0, length(IH)))
for (i in 1:length(IH)) {
  shapiro_p_IH$name[i] <- colnames(data_Q_total[IH[i]])
  shapiro_p_IH$p_value[i] <- shapiro.test(data_Q_total[, IH[i]])["p.value"][[1]]
}

# CFA
round(cor(data_Q_total[, IH], 
          method = "spearman",
          use = "pairwise.complete.obs"), 2)
modelIH <- 'IH1 =~ IH1rev1 + IH2rev1 + IH3rev1 + IH4rev1 + IH5rev1
            IH2 =~ IH6norm2 + IH7norm2 + IH8norm2 + IH9norm2 + IH10norm2
            IH3 =~ IH11norm3 + IH12norm3 + IH13norm3 + IH14norm3 + IH15norm3 + IH16norm3
            IH4 =~ IH17rev4 + IH18rev4 + IH19rev4 + IH20rev4 + IH21rev4 + IH22rev4
IH1 ~~ IH2
IH1 ~~ IH3
IH1 ~~ IH4
IH2 ~~ IH3
...
# Describe: skew, kurtosis, mean
describe(data_Q_total[, IH_sum])[, c("mean", "skew", "kurtosis")]
# Shapiro-Wilks test
shapiro_p_IH_sum <- data.frame(name = rep(0, length(IH_sum)), 
                               p_value = rep(0, length(IH_sum)))
for (i in 1:length(IH_sum)) {
  shapiro_p_IH_sum$name[i] <- colnames(data_Q_total[IH_sum[i]])
  shapiro_p_IH_sum$p_value[i] <- shapiro.test(data_Q_total[, IH_sum[i]])["p.value"][[1]]
}
...
# NOTE: who needs to be excluded here?
...
# Describe: skew, kurtosis, mean
describe(data_Q_total[, need_clo])[, c("mean", "skew", "kurtosis")]
# Shapiro-Wilks test
shapiro_p_need_clo <- data.frame(name = rep(0, length(need_clo)), 
                                 p_value = rep(0, length(need_clo)))
for (i in 1:length(need_clo)) {
  shapiro_p_need_clo$name[i] <- colnames(data_Q_total[need_clo[i]])
  shapiro_p_need_clo$p_value[i] <- shapiro.test(data_Q_total[, need_clo[i]])["p.value"][[1]]
}
...
# CFA
round(cor(data_Q_total[, need_clo], 
          method = "spearman",
          use = "pairwise.complete.obs"), 2)
...
# Factor/sum score level assumptions
describe(data_Q_total$CloSum)[c("mean", "skew", "kurtosis")]
...
# Cronbach's alpha
NeedforClos <- data_Q_total[, need_clo]
alpha(NeedforClos)
...
need_cog <- c(which(colnames(data_Q_total) == "cognition1"),
              which(colnames(data_Q_total) == "cognition2"),
              which(colnames(data_Q_total) == "cognition3.rev"),
              which(colnames(data_Q_total) == "cognition4.rev"),
              which(colnames(data_Q_total) == "cognition5.rev"),
              which(colnames(data_Q_total) == "cognition6"),
              which(colnames(data_Q_total) == "cognition7.rev"),
              which(colnames(data_Q_total) == "cognition8.rev"),
              which(colnames(data_Q_total) == "cognition9.rev"),
              which(colnames(data_Q_total) == "cognition10"),
              which(colnames(data_Q_total) == "cognition11"),
              which(colnames(data_Q_total) == "cognition12.rev"),
              which(colnames(data_Q_total) == "cognition13"),
              which(colnames(data_Q_total) == "cognition14"),
              which(colnames(data_Q_total) == "cognition15"),
              which(colnames(data_Q_total) == "cognition16.rev"),
              which(colnames(data_Q_total) == "cognition17.rev"),
              which(colnames(data_Q_total) == "cognition18"))

need_cog_rev <- c(which(colnames(data_Q_total) == "cognition3.rev"),
                  which(colnames(data_Q_total) == "cognition4.rev"),
                  which(colnames(data_Q_total) == "cognition5.rev"),
                  which(colnames(data_Q_total) == "cognition7.rev"),
                  which(colnames(data_Q_total) == "cognition8.rev"),
...
data_Q_total <- data_Q_total %>% 
  mutate(Cog_omit = case_when(Cog_max >= .95 | END.QUESTIONNAIRE.NEED.COG <= 1000*length(need_cog) ~ 1,
                             Cog_max < .95 & END.QUESTIONNAIRE.NEED.COG > 1000*length(need_cog) ~ 0))
 
# Omit Cog data based on the Cog_omit column
data_Q_total[which(data_Q_total$Cog_omit == 1), need_cog] <- NA
# NOTE: if we want to omit participants with only NAs, we use Cog_omit != 0
...
# Describe: skew, kurtosis, mean
describe(data_Q_total[, need_cog])[, c("mean", "skew", "kurtosis")]
# Shapiro-Wilks test
shapiro_p_need_cog <- data.frame(name = rep(0, length(need_cog)), 
                                 p_value = rep(0, length(need_cog)))
for (i in 1:length(need_cog)) {
  shapiro_p_need_cog$name[i] <- colnames(data_Q_total[need_cog[i]])
  shapiro_p_need_cog$p_value[i] <- shapiro.test(data_Q_total[, need_cog[i]])["p.value"][[1]]
}

# CFA
round(cor(data_Q_total[, need_cog], 
          method = "spearman",
          use = "pairwise.complete.obs"), 2)
modelCog <- 'Cognition =~ cognition1 + cognition2 + cognition3.rev + cognition4.rev + cognition5.rev + cognition6 + cognition7.rev + cognition8.rev + cognition9.rev + cognition10 + cognition11 + cognition12.rev + cognition13 + cognition14 + cognition15 + cognition16.rev + cognition17.rev + cognition18'
modelCog.est <- cfa(modelCog, 
                    data = data_Q_total, 
                    std.lv = TRUE)
...
# Factor/sum score level assumptions
describe(data_Q_total$CogSum)[c("mean", "skew", "kurtosis")]
...
# Cronbach's alpha
NeedforCog <- data_Q_total[, need_cog]
alpha(NeedforCog)
...
data_Q_total <- data_Q_total %>% 
  mutate(AOT_omit = case_when(AOT_max >= .95 | END.QUESTIONNAIRE.OPEN.THINK <= 1000*length(aot) ~ 1,
                             AOT_max < .95 & END.QUESTIONNAIRE.OPEN.THINK > 1000*length(aot) ~ 0))
 
# Omit AOT data based on the AOT_omit column
data_Q_total[which(data_Q_total$AOT_omit == 1), aot] <- NA
# NOTE: if we want to omit participants with only NAs, we use AOT_omit != 0
...
# Describe: skew, kurtosis, mean
describe(data_Q_total[, aot])[, c("mean", "skew", "kurtosis")]
# Shapiro-Wilks test
shapiro_p_aot <- data.frame(name = rep(0, length(aot)), 
                            p_value = rep(0, length(aot)))
for (i in 1:length(aot)) {
  shapiro_p_aot$name[i] <- colnames(data_Q_total[aot[i]])
  shapiro_p_aot$p_value[i] <- shapiro.test(data_Q_total[, aot[i]])["p.value"][[1]]
}

# CFA
round(cor(data_Q_total[, aot], 
          method = "spearman",
          use = "pairwise.complete.obs"), 2)
modelAOT <- 'AOT =~ openNormal1 + openNormal2 + openNormal3 + openReverse1 + openReverse2 + openReverse3 + openReverse4'
modelAOT.est <- cfa(modelAOT, 
                    data = data_Q_total, 
                    std.lv = TRUE) 
...
# Factor/sum score level
describe(data_Q_total$AOTSum)[c("mean", "skew", "kurtosis")]
...
# Cronbach's alpha
AOT <- data_Q_total[, aot]
alpha(AOT)

```

NOTE: we need to think about what we do with missing data (i.e., actually missing data and not data that we have now marked as NA due to exclusion criteria) / we need to remember that these are separate things
...
# Create a separate df for subsetting purposes
...
# Omit specific participants who commented that they had technical issues with this task or did not understand the task
data_matreasT_final <- data_matreasT_final %>%
...
describe(data_matreasT_final$MatrixCorrectCount)[c("mean", "skew", "kurtosis")]
shapiro.test(data_matreasT_final$MatrixCorrectCount)
