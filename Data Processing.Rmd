...
For analyses that required no missing data, they also imputed their matrix; Is this something we wanna consider? -> nope
...
# MILLA: leave both thresholds in :))
...
# Exclusion Criteria: Participant-Level
  # MILLA: omit median RT (if we keep trial-level RT criterion), keep 60% accuracy here at least
  # <=200ms median RT
  # <=60% accuracy
data_nogoT <- data_nogoT %>%
  group_by(Participant.Private.ID) %>% 
...
# Exclusion Criteria: Trial-Level
# MILLA: max RT -> delete; min RT 150ms ADD
...
# Exclusion Criteria: Participant-Level
# MILLA: omit median RT, keep 60% accuracy
  # <= 50ms median RT
  # <= 60% accuracy
...
Exclusion criteria: none for now (keep this)
