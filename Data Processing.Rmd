...
# McDonald's omegas
summary(omega(data_Q_total[, sci_att]))
...
#first select only ppts with some missing data
data_Q_sub_missing <- data_Q_sub %>%
  filter(q_missingdata > 0)

gg_miss_fct(x = data_Q_sub_missing, fct = Age)
cor.test(data_Q_sub_missing$Age, data_Q_sub_missing$q_missingdata, 
...
#check missing data correlation with age                 
#first select only ppts with some missing data
data_T_sub_missing <- data_T_sub %>%
  filter(q_missingdata > 0)

gg_miss_fct(x = data_T_sub_missing, fct = Age)
cor.test(data_T_sub_missing$Age, data_T_sub_missing$q_missingdata, 
         method = "spearman", exact = FALSE)
...
# Save data
write.csv(data_efa, "C:/Users/Omistaja/OneDrive/Tiedostot/GitHub/Scientific-thinking-and-decision-making/data_efa.csv", row.names = FALSE)
...
# To impute or not to impute? 127 participants with no missing data, 139 overall
