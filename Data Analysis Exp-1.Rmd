...
#levels(demo$Education)[levels(demo$Education)=="Muu tutkinto, mikÃ¤?"] <- 7
...
data_citiT_wide <- pivot_wider(data = data_citiT_final,
                               names_from = src_id,
                               values_from = c(src_words, src_rt, RT_WPM, source_type, source_quality, Shareability_src, Convincingness_src, Expertise_src, Reliability_src, SAMintensity_final, SAMvalence_final, Curiosity_pet, Interest_pet, Familiarity_pet, Support_pet))
...
data_long$source_type <- as.factor(data_long$source_type)
...
#Convincingness
m1_conv <- lmer(Convincingness_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data = data_long) 
...
library(afex)
m1_conv_sums <- mixed(Convincingness_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data_long, check_contrasts=TRUE)
...
leveneTest(Convincingness_src ~ source_type*source_quality, data = data_long)

#Expertise
m1_exp <-lmer(Expertise_src ~ source_type*source_quality + Age + Education +  (1 | Participant.Private.ID),data = data_long) 

#refit
m1_exp_sums <- mixed(Expertise_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data_long, check.contrasts=TRUE)
...
leveneTest(Expertise_src ~ source_type*source_quality, data = data_long)

#Reliability
m1_cred <-lmer(Reliability_src ~ source_type*source_quality + Age + Education +  (1 | Participant.Private.ID),data = data_long) 
  
m1_cred_sums <- mixed(Reliability_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data_long, check.contrasts=TRUE)
...
leveneTest(Reliability_src ~ source_type*source_quality, data = data_long)
...
df %>% 
  group_by(source_type, source_quality) %>% 
...
convplot <- df2 %>% 
  ...
  aes(x = source_type, y = df_groups, color = source_quality, shape = source_quality) +
  geom_line(aes(group = source_quality)) +
  geom_point() +
  ylim(1, 5) +
  geom_linerange(aes(x = source_type, ymin = df_groups - df_sem, ymax = df_groups + df_sem), size = .3) +
...
m1_conv_orig <- lmer(Convincingness_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data = data_long) 

m1_conv_length <- lmer(Convincingness_src ~ num_words + Age + Education + (1|Participant.Private.ID),data = data_long) 

m1_conv_type <- lmer(Convincingness_src ~ source_type*num_words + Age + Education + (1|Participant.Private.ID),data = data_long) 
...
#define the series of models to compare
m1_conv_orig_ML <- lmer(Convincingness_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 

m1_conv_length_ML <- lmer(Convincingness_src ~ num_words + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 

m1_conv_type_ML <- lmer(Convincingness_src ~ source_type*num_words + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 
...
#For expertise
m1_exp_orig <- lmer(Expertise_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data = data_long) 

m1_exp_length <- lmer(Expertise_src ~ num_words + Age + Education + (1|Participant.Private.ID),data = data_long) 

m1_exp_type <- lmer(Expertise_src ~ source_type*num_words + Age + Education + (1|Participant.Private.ID),data = data_long) 
...
#define the series of models to compare
m1_exp_orig_ML <- lmer(Expertise_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 

m1_exp_length_ML <- lmer(Expertise_src ~ num_words + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 

m1_exp_type_ML <- lmer(Expertise_src ~ source_type*num_words + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 
...
#Reliability
m1_cred_mod1 <-lmer(Reliability_src ~ source_type*source_quality + text_length + Age + Education + (1 | Participant.Private.ID),data = data_long) 

summary(m1_cred_mod1)

#define the series of models to compare
m1_cred_orig_ML <- lmer(Reliability_src ~ source_type*source_quality + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 

m1_cred_length_ML <- lmer(Reliability_src ~ num_words + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 

m1_cred_type_ML <- lmer(Reliability_src ~ source_type*num_words + Age + Education + (1|Participant.Private.ID),data = data_long,REML=FALSE) 
...
res<-compareGroups(source_type ~ source_quality + RT_WPM + gender + scale_Age + Education, data=data_long)
...
emmip(m1_conv, source_quality ~ source_type)

#calculate estimated marginal means for each combination of source type and quality
conv.emm <- emmeans(m1_conv, ~ source_type * source_quality,weights = "proportional")
exp.emm <- emmeans(m1_exp, ~ source_type * source_quality,weights = "proportional")
cred.emm <- emmeans(m1_cred, ~ source_type * source_quality,weights = "proportional")
...
m1_read <- glmer(formula = RT_WPM ~ source_type*source_quality + scale_Age + Education + (1 | Participant.Private.ID),
...
m_sums <- mixed(RT_WPM ~ source_type*source_quality + scale_Age + Education + (1 | Participant.Private.ID),
                          family = Gamma(link = "log"),
...
##Topic familiarity
m1_fam <- lmer(Familiarity_pet ~ time*source_type*source_quality + Age + Education + (1 | Participant.Private.ID),data = data_long)

summary(m1_fam)

m1_fam_sums <- mixed(Familiarity_pet ~ time*source_type*source_quality + Age + Education + (1 | Participant.Private.ID),data_long, check_contrasts=TRUE)
...
emmip(m1_fam, source_type ~ time|source_quality, type = "response") +
...
#Topic interest
m1_int <- lmer(Interest_pet ~ time*source_type*source_quality + Age +  Education + (1 | Participant.Private.ID),data = data_long)

summary(m1_int)

m1_int_sums <- mixed(Interest_pet ~ time*source_type*source_quality + Age + Education + (1 | Participant.Private.ID),data_long, check_contrasts=TRUE)

m1_int_sums$anova_table

#Topic curiosity
m1_cur <- lmer(Curiosity_pet ~ time*source_type*source_quality + Age +  Education + (1 | Participant.Private.ID),data = data_long)

summary(m1_cur)

m1_cur_sums <- mixed(Curiosity_pet ~ time*source_type*source_quality + Age + Education + (1 | Participant.Private.ID),data_long, check_contrasts=TRUE)
...
efa_CI$source_type <- as.factor(efa_CI$source_type)
...
2.1. High curiosity factor scores will interact with source_type and source quality to predict longer reading times and higher ratings of expertise and reliability for authority and reliable sources, higher ratings of topic interest and topic curiosity, and more positive emotional valence for all sources.  
...
#on expertise
m_cur_exp <- lmer(Expertise_src ~ source_type*source_quality*f2 + Age + Education +  (1 | Participant.Private.ID), data = efa_CI)

summary(m_cur_exp)

m_cur_exp_sums <- mixed(Expertise_src ~ source_type*source_quality*f2 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)
...
leveneTest(Expertise_src ~ source_type*source_quality*f2 + Age + gender + Education +  (1 | Participant.Private.ID), data = data_long)

#on reliability
m_cur_rel <- lmer(Reliability_src ~ source_type*source_quality*f2 + Age + Education +  (1 | Participant.Private.ID), data = efa_CI)

summary(m_cur_rel)


m_cur_rel_sums <- mixed(Reliability_src ~ source_type*source_quality*f2 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)
...
m_cur_val <- lmer(SAMvalence_final ~ source_type*source_quality*f2 + Age +  Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_cur_val)

m_cur_val_sums <- mixed(SAMvalence_final ~ source_type*source_quality*f2 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)

 m_cur_val_sums$anova_table
 
 #exploratory non-prereg model 
 #on emotional AROUSAL
m_cur_aro <- lmer(SAMintensity_final ~ source_type*source_quality*f2 + Age +  Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_cur_aro)

m_cur_aro_sums <- mixed(SAMintensity_final ~ source_type*source_quality*f2 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)

 m_cur_aro_sums$anova_table

#on topic interest
m_cur_int <- lmer(Interest_pet ~ source_type*source_quality*f2 + Age + Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_cur_int)

m_cur_int_sums <- mixed(Interest_pet ~ source_type*source_quality*f2 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)

 m_cur_int_sums$anova_table

#on topic curiosity
m_cur_cur <- lmer(Curiosity_pet ~ source_type*source_quality*f2 + Age + Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_cur_cur)

m_cur_cur_sums <- mixed(Curiosity_pet ~ source_type*source_quality*f2 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)
...
m_cur_read <- glmer(formula = RT_WPM ~ source_type*source_quality*f2 + scale_Age + Education + (1 | Participant.Private.ID),
                          family = Gamma(link = "log"),
                          data = efa_CI,
...
m_sums <- mixed(RT_WPM ~ source_type*source_quality*scale_f2 + scale_Age + Education + (1 | Participant.Private.ID),
                          family = Gamma(link = "log"),
...
#on convincingness
m_pers_conv <- lmer(Convincingness_src ~ source_type*source_quality*f1 + Age + Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_pers_conv)

m_pers_conv_sums <- mixed(Convincingness_src ~ source_type*source_quality*f1 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)

 m_pers_conv_sums$anova_table

#on shareability
m_pers_share <- lmer(Shareability_src ~ source_type*source_quality*f1 + Age  + Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_pers_share)

m_pers_share_sums <- mixed(Shareability_src ~ source_type*source_quality*f1 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)

 m_pers_share_sums$anova_table

#on arousal
m_pers_aro <- lmer(SAMintensity_final ~ source_type*source_quality*f1 + Age +  Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_pers_aro)

m_pers_aro_sums <- mixed(SAMintensity_final ~ source_type*source_quality*f1 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)
...
#on expertise
m_open_expert <- lmer(Expertise_src ~ source_type*source_quality*f4 + Age + Education + (1 | Participant.Private.ID), data = efa_CI) 

summary(m_open_expert)

#on reliability
m_open_reliab <- lmer(Reliability_src ~ source_type*source_quality*f4 + Age + Education + (1 | Participant.Private.ID), data = efa_CI)

summary(m_open_reliab)

#on reading times
m_open_read <- glmer(formula = RT_WPM ~ source_type*source_quality*f4 + scale_Age + Education + (1 | Participant.Private.ID),
                          family = Gamma(link = "log"),
                          data = efa_CI,
                          #nAGQ = 0,
                          glmerControl(optimizer="bobyqa",
...
##on expertise
m_cog_exp <- lmer(Expertise_src ~ source_type*source_quality*f3 + Age + Education +  (1 | Participant.Private.ID), data = efa_CI)

summary(m_cog_exp)

m_cog_exp_sums <- mixed(Expertise_src ~ source_type*source_quality*f3 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)

 m_cog_exp_sums$anova_table

###on reliability
m_cog_rel <- lmer(Reliability_src ~ source_type*source_quality*f3 + Age + Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_cog_rel)

m_cog_rel_sums <- mixed(Reliability_src ~ source_type*source_quality*f3 + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)
...
#UUt on expertise
m_uut_exp <- lmer(Expertise_src ~ source_type*source_quality*FlexSum + Age + Education + (1 | Participant.Private.ID), data = efa_CI) 

summary(m_uut_exp)

m_uut_exp_sums <- mixed(Expertise_src ~ source_type*source_quality*FlexSum + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)

m_uut_exp_sums$anova_table

#uut on reliability
m_uut_rel <- lmer(Reliability_src ~ source_type*source_quality*FlexSum + Age + Education + (1 | Participant.Private.ID), data = efa_CI) 

summary(m_uut_rel)

m_uut_rel_sums <- mixed(Reliability_src ~ source_type*source_quality*FlexSum + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)
...
m_uut_read <- glmer(formula = RT_WPM ~ source_type*source_quality*FlexSum + scale_Age + Education + (1 | Participant.Private.ID),
                          family = Gamma(link = "log"),
                          data = efa_CI,
                          nAGQ = 0,
                          glmerControl(optimizer="bobyqa",
...
#expertise
m_d_exp <- lmer(Expertise_src ~ source_type*source_quality*GNGdprime + Age + Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_d_exp)

m_d_exp_sums <- mixed(Expertise_src ~ source_type*source_quality*GNGdprime + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)

 m_d_exp_sums$anova_table

#reliability
m_d_rel <- lmer(Reliability_src ~ source_type*source_quality*GNGdprime + Age + Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_d_rel)

m_d_rel_sums <- mixed(Reliability_src ~ source_type*source_quality*GNGdprime + Age + Education + (1 | Participant.Private.ID),efa_CI, check_contrasts=TRUE)
...
#trying the code found here works to identify an optimizer that works https://rstudio-pubs-static.s3.amazonaws.com/33653_57fc7b8e5d484c909b615d8633c01d51.html 
m_d_read <- glmer(formula = RT_WPM ~ source_type*source_quality*GNGdprime + Age + Education + (1 | Participant.Private.ID),
                          family = Gamma(link = "log"),
                          data = efa_CI,
...
#this one works
m_d_read_sums <- mixed(RT_WPM ~ source_type*source_quality*GNGdprime + Age + Education + (1 | Participant.Private.ID),
                          family = Gamma(link = "log"),
...
#bias
m_bias_conv <- lmer(Convincingness_src ~ source_type*source_quality*GNGbetalog + Age + gender + Education +  (1 | Participant.Private.ID), data = efa_CI) 

summary(m_bias_conv)

#shareability
m_GNGbetalog_share <- lmer(Shareability_src ~ source_type*source_quality*GNGbetalog + Age + gender + Education +  (1 | Participant.Private.ID), data = efa_CI) 
...
ggplot(plotdata,aes(x=f2,y=RT_WPM,color=source_type))+geom_point()+
  aes(color = source_type) +
...
ggplot(plotdata,aes(x=GNGdprime,y=RT_WPM,color=source_type))+geom_point()+
  aes(color = source_type) +
stat_smooth(method="lm",se=TRUE) +
  scale_color_discrete(labels = c("Authority", "Personal")) + 
